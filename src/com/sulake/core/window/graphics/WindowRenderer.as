package com.sulake.core.window.graphics
{
   ...
   public class WindowRenderer implements IWindowRenderer
   {
      ...
      public function WindowRenderer(param1:ISkinContainer)
      {
         ...
         var_663 = new Array();
         var_1653 = new Point();
         var_1654 = new Rectangle();
         var_333 = new Rectangle();
         var_334 = new Dictionary(true);
         var_708 = param1;
      ...
      {
         var _loc2_:WindowRendererItem = var_334[param1] as WindowRendererItem;
      ...
      public function render() : void
      {
         ...
         while(_loc4_ < _loc2_)
         {
            _loc1_ = var_663.pop() as WindowController;
            if(!_loc1_.disposed)
            {
               _loc3_ = getWindowRendererItem(_loc1_);
               var_333.x = _loc3_.dirty.x;
               var_333.y = _loc3_.dirty.y;
               var_333.width = _loc3_.dirty.width;
               var_333.height = _loc3_.dirty.height;
               renderWindowBranch(_loc1_,_loc3_,var_333);
            }
            _loc4_++;
         }
      }
      ...
      {
         param1.removeEventListener(WindowDisposeEvent.const_650,windowDisposedCallback);
         var _loc2_:WindowRendererItem = var_334[param1] as WindowRendererItem;
         if(_loc2_ != null)
         {
            _loc2_.dispose();
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            _disposed = true;
            var_663 = null;
            for(_loc1_ in var_334)
            {
               delete var_334[_loc1_];
            }
            var_334 = null;
      ...
      private function renderWindowBranch(param1:WindowController, param2:WindowRendererItem, param3:Rectangle) : void
      {
         ...
         if(param1.visible)
         {
            if(getDrawLocationAndClipRegion(param1,param3,var_1653,var_1654))
            {
               if(param2.render(param1,var_1653,var_1654))
               {
      ...
      public function registerRenderable(param1:IWindow) : void
      {
         ...
         if(_loc2_ == null)
         {
            _loc2_ = new WindowRendererItem(this,_drawBufferAllocator,var_708);
            var_334[param1] = _loc2_;
            param1.addEventListener(WindowDisposeEvent.const_650,windowDisposedCallback);
         }
      }
      ...
      public function addToRenderQueue(param1:IWindow, param2:Rectangle, param3:uint) : void
      {
         ...
         if(getWindowRendererItem(param1).invalidate(param1,param2,param3))
         {
            ...
            var_333.x = param2.x - param1.x;
            var_333.y = param2.y - param1.y;
            var_333.width = param2.width;
            var_333.height = param2.height;
            if(param1.testParamFlag(WindowParam.const_29))
            {
               while(param1.testParamFlag(WindowParam.const_29))
               {
                  ...
                  var_333.offset(param1.x,param1.y);
                  param1 = _loc4_;
               }
            }
            getWindowRendererItem(param1).invalidate(param1,var_333,WindowRedrawFlag.const_293);
            if(var_663.indexOf(param1) == -1)
            {
               var_663.push(param1);
            }
         }

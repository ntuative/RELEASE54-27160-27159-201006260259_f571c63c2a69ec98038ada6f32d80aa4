package com.sulake.habbo.friendlist
{
   ...
   public class FriendRequestsView implements ITabView, IFriendRequestsView
   {
      ...
      private static const const_1461:String = "no_reqs_info";
       
      
      private var var_1538:IContainerButtonWindow;
      ...
      private function onAcceptAllButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            if(_loc4_.state != FriendRequest.const_333 && _loc4_.state != FriendRequest.const_347)
            {
               ...
               {
                  _friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_50,_loc4_.requestId));
               }
            }
         }
      ...
      {
         var_1538 = IContainerButtonWindow(param1.findChildByName("accept_all_but"));
         var_1539 = IContainerButtonWindow(param1.findChildByName("reject_all_but"));
         var_1539.procedure = onDismissAllButtonClick;
         var_1538.procedure = onAcceptAllButtonClick;
         refreshButtons();
      }
      ...
      private function onDismissAllButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         for each(_loc4_ in _friendList.friendRequests.requests)
         {
            if(_loc4_.state != FriendRequest.const_333 && _loc4_.state != FriendRequest.const_347)
            {
               _loc4_.state = FriendRequest.const_347;
               refreshRequestEntry(_loc4_);
               if(_friendList && false)
               {
                  _friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_256,_loc4_.requestId));
               }
            }
         }
         refresh();
      ...
      public function refreshRequestEntry(param1:FriendRequest) : void
      {
         ...
         else if(param1.state == FriendRequest.const_347)
         {
            _friendList.refreshText(_loc2_,"info_text",true,"${friendlist.request.declined}");
         }
         else if(param1.state == FriendRequest.const_902)
         {
      ...
      private function refreshButtons() : void
      {
         ...
         setEnabled(var_1538,_loc1_);
         setEnabled(var_1539,_loc1_);
      }
      ...
      public function acceptRequest(param1:int) : void
      {
         ...
         if(_friendList && false)
         {
            _friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_50,param1));
         }
      }
      ...
      public function declineRequest(param1:int) : void
      {
         ...
         _loc2_.state = FriendRequest.const_347;
         var _loc3_:DeclineBuddyMessageComposer = new DeclineBuddyMessageComposer();
         ...
         if(_friendList && false)
         {
            _friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_256,param1));
         }
      }
   }
}

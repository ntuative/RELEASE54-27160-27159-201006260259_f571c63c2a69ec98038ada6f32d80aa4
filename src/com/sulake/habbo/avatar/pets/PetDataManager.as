package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager
   {
      ...
      private var var_144:AvatarRenderManager;
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(var_445.getValue(_loc4_) == null)
               {
                  var_445.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(var_1126.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(var_1125.indexOf(_loc6_) == -1)
               {
                  ...
                  var_263.push(_loc8_);
                  var_1125.push(_loc6_);
                  if(param3 != null && var_719.indexOf(param3) == -1)
                  {
                     var_719.push(param3);
                  }
               }
      ...
      {
         var _loc3_:IPetData = var_445.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
            assetsReady(param1,AvatarScaleType.const_46,param2);
         }
         return _loc3_;
      }
      ...
      private function notifyListeners() : void
      {
         ...
         while(false)
         {
            _loc1_ = var_719.pop() as IPetDataListener;
            if(_loc1_ != null)
            {
               _loc1_.petDataReady();
            }
      ...
      {
         var _loc4_:PetData = var_445.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = var_445.getKeys();
         for each(key in speciesKeys)
         {
            petData = var_445.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.var_224.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
